...
pub fn get_longest_common_subset<'a>(source: &str, candidates: &Vec<&'a str>) -> Vec<&'a str> {
    let mut len_list = Vec::new();
    for c in candidates.iter() {
        let len = calc_common_subset_len(source, c);
        len_list.push(len);
    }
...
fn calc_common_subset_len(source: &str, candidate: &str) -> u32 {
    ...
    for i in 0..source_len {
        for j in 0..candidate_len {
            let left = if j >= 1 { char_table[[i, j - 1]] } else { 0 };
            let up = if i >= 1 { char_table[[i - 1, j]] } else { 0 };
            let left_up = if j >= 1 && i >= 1 {
                char_table[[i - 1, j - 1]]
            } else {
                0
            };
            if source.chars().nth(i).unwrap() == candidate.chars().nth(j).unwrap() {
                char_table[[i, j]] = left_up + 1;
            } else {
                char_table[[i, j]] = if left >= up { left } else { up };
            }
